...
//1) Définir les objets de base sur lesquels va s'appuyer la suite
	...
	class canal {
		...
		public:
			...
			string txt;         //Déclarer le texte qui reste à lire  					 //OU PEUT-ÊTRE UTILISER OBJET DE CLASSE VECT???????
	...
	//iv) Fonction : extract ; extrait une valeur double d'une suite de caractères encadrés par '§' (pour extraire les codes spéciaux)
	double extract(string str){
		int lgstr = length(str)       	//Extraire la longueur du string 		
		string nbonly;                  //Initier un nouveau string, dans lequel insérer seulement les chiffres
		int a = 2;                      //Commencer à la position [2], le [0] étant occupé par '§' et le [1] par le type de valeur à extraire
		while(str[a]!='§'&a<=lgstr) {nbonly += str[a];}
	...
	//v) Objet : canaux ; assemblage de tous les canaux utilisés
	vect<canal> canaux;
...
read(canal can) {
	...
	if(can.txt[0]=='§'){
		if(can.txt[1]=='p'){      //'p' pour "pause" -> ajouter une pause à la lecture
			double val = extract(can.txt);       //Extraire le temps que durera la pause
			...
			//Extraire la nouvelle vitesse
			double val = extract(can.txt);
			can.vit = val;
...
////////										//FLAG POUR POSITION IMPOSSIBLE; PEUT-ÊTRE QUE ÇA VA EMMENER UN BUG ÉVENTUELLEMENT?
	...
	if(can.txt[0]!='\n') {
		curspos(can.posx,can.posy-base.consy) ; out(can.txt[0]) ;     //Inscrire dans la console
													//ICI, CURSPOS() ET OUT() SONT DES FONCTIONS QUI SONT ENCORE À DÉFINIR
