...
//1) Créer la classe
	template <class Type> //Créer un "template" de type, qui pourra être des int, des char... ce que je veux! 
		...
		//Constructeurs	
			...
			vect<Type>(Type nxt[], int nbpos) {nb=nbpos ; pt = new Type [nbpos] ;	for(int pos=0; pos < nbpos; pos++) pt[pos] = nxt[pos];}  //Créer un constructeur pour créer à partir d'un array
		//Destructeur
			~vect<Type>() {delete[] pt;}
...
//2) Créer la classe-fille avec des strings
	...
	class StringAsVect : public vect<char> {
		public:
	//Récupérer les constructeurs de la classe de base				
		//using vect<char>::vect;  //Ligne spéciale censée hériter les constructeurs (tous) de la classe de base ; n'hérite pas le (void)???? 
		//Donc le faire manuellement:
		StringAsVect() {nb=0 ; pt = new char;}  //Créer un constructeur par défaut, pour initialiser tous les paramètres
		StringAsVect(char nxt) {nb=1 ; pt = new char [1]; pt[0] = nxt;}  //Créer un constructeur pour créer l'objet avec une seule valeur
		StringAsVect(char nxt[], int nbpos) {nb=nbpos ; pt = new char [nbpos] ;	for(int pos=0; pos < nbpos; pos++) pt[pos] = nxt[pos];}  //Créer un constructeur pour créer à partir d'un array
	//Constructeur spécialisé
		StringAsVect (std::string str) {
			nb = str.length();
			pt = new char [nb];
